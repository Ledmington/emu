...
public final class Main {
    ...
    public static void main(final String[] args) {
        ...
        if (displayHeader) {
            ...
            out.println("  Class:  " + (elf.getFileHeader().is32Bit() ? "ELF32" : "ELF64"));
            out.println("  Data:  "
                    + (elf.getFileHeader().isLittleEndian()
                            ? "2's complement, little-endian"
                            : "2's complement, big-endian"));
            out.println("  Version:  1 (current)");
            out.println("  OS/ABI: " + elf.getFileHeader().getOSABI().getName());
            out.println("  ABI Version: " + elf.getFileHeader().getABIVersion());
            out.println("  Type: " + elf.getFileHeader().getFileType().getName());
            out.println("  Machine: " + elf.getFileHeader().getISA().getName());
            out.printf("  Version: 0x%x\n", elf.getFileHeader().getVersion());
            out.printf("  Entry point address: 0x%x\n", elf.getFileHeader().getEntryPointVirtualAddress());
            out.printf(
